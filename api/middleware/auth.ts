...
export class AuthMiddleware {
  ...
  static verifyToken(
    ...
    try {
      ...
      mylogger.warn("Invalid token", { error: error.message });
  ...
  static async verifySession(
    ...
    try {
      ...
      mylogger.error("Session verification failed", { error: error.message });
  ...
  static requireRole(roles: string | string[]) {
    ...
    return (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
      ...
      if (!req.user.role || !allowedRoles.includes(req.user.role)) {
        mylogger.warn("Insufficient permissions", {
          userId: req.user.id,
  ...
  static requirePlayer: RequestHandler = (req, res, next) => {
  ...
  static optionalAuth(
    ...
    try {
      ...
      mylogger.debug("Optional auth failed", { error: error.message });
    }
...
export class AuthUtils {
  ...
  static generateSessionToken(): string {
    return jwt.sign({ type: "session", timestamp: Date.now() }, JWT_SECRET, {
      expiresIn: "7d",
    });
  }
}
