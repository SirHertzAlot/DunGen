...
const terrainLogger = logger.create({ module: 'TerrainConfigManager' });
...
export class TerrainConfigManager {
  ...
  private reloadIfNeeded(): void {
    try {
      if (!fs.existsSync(this.configPath)) {
        terrainLogger.error('Terrain configuration file not found: ' + this.configPath);
        return;
      }
      ...
      if (stats.mtime.getTime() > this.lastModified) {
        ...
        terrainLogger.info('Terrain configuration reloaded: ' + this.configPath);
      }
    } catch (error: any) {
      terrainLogger.error('Error checking terrain config file: ' + error.message);
    }
  }
  ...
  private loadConfig(): void {
    try {
      ...
      terrainLogger.info(`Terrain configuration loaded successfully: ${Object.keys(parsedConfig.terrain_types).length} terrain types from ${this.configPath}`);
    } catch (error: any) {
      terrainLogger.error(`Failed to load terrain configuration from ${this.configPath}: ${error.message}`);
      throw error;
    }
  }
