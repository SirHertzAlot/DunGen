...
export class ProperTerrainGenerator {
  ...
  public generateChunk(
    ...
    const heightmap = this.generateHeightmapWithNoise(
      chunkX,
      chunkZ,
      size,
      biome,
    ...
    log.info("Generated terrain chunk with THREE.Terrain", {
  ...
  private generateHeightmapWithNoise(
    chunkX: number,
    chunkZ: number,
    size: number,
    biome: BiomeType,
    ...
    switch (biome.type) {
      case "mountain":
        ...
        const ridgeNoise = Math.abs(
          Math.sin(x + seed) * Math.cos(z + seed * 1.3),
        );
      ...
      case "desert":
        ...
        const dune2 =
          Math.sin(x * 1.5 + seed * 2) * Math.cos(z * 1.2 + seed * 2) * 0.3;
      ...
      case "forest":
        ...
        const hill2 =
          Math.sin(x * 2.1 + seed * 3) * Math.cos(z * 1.8 + seed * 3) * 0.5;
      ...
      case "swamp":
        ...
        const mound =
          Math.max(
            0,
            Math.sin(x * 0.1 + seed) * Math.cos(z * 0.1 + seed) - 0.7,
          ) * 2;
  ...
  private applyBiomeModifications(
    heightmap: number[][],
    biome: BiomeType,
    chunkX: number,
    chunkZ: number,
      ...
      for (let x = 0; x < size; x++) {
        ...
        switch (biome.type) {
          case "mountain":
            ...
            const ridgeNoise =
              Math.sin(worldX * 0.01) * Math.cos(worldZ * 0.01);
          ...
          case "desert":
            // Add sand dunes
            const duneNoise =
              Math.sin(worldX * 0.02) + Math.cos(worldZ * 0.015);
            height += duneNoise * 8;
            break;
          ...
          case "forest":
            // Add rolling hills
            const hillNoise =
              Math.sin(worldX * 0.005) * Math.cos(worldZ * 0.007);
            height += hillNoise * 12;
            break;
          ...
          case "swamp":
            ...
            height *= 0.6;
            const moundNoise =
              Math.sin(worldX * 0.03) * Math.cos(worldZ * 0.03);
            if (moundNoise > 0.7) {
              height += moundNoise * 5;
            }
            break;
  ...
  private sampleNoise(x: number, z: number, frequency: number): number {
    ...
    const value =
      Math.sin(x * frequency + this.seed) *
      Math.cos(z * frequency + this.seed * 1.3);
    return (value + 1) / 2; // Normalize to 0-1
  }

  private hashChunk(chunkX: number, chunkZ: number): number {
    return Math.abs(
      (chunkX * 73856093 + chunkZ * 19349663 + this.seed) % 1000000,
    );
  }
