...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/players/:playerId/session", async (req, res) => {
    ...
    try {
      ...
      const regions = await storage.getAllRegions();
      const region =
        regions.find((r) => r.id === req.body.regionId) || regions[0];

      const sessionData = {
        playerId: req.params.playerId,
        sessionToken: uuidv4(),
        regionId: region?.id || uuidv4(),
        unificationContainerId:
          region?.unificationContainerId || "default_container",
  ...
  app.post("/api/infrastructure/reconfigure", async (req, res) => {
    ...
    try {
      ...
      await infrastructureManager.reconfigureNode(
        nodeType,
        environment,
        config,
      );
    ...
    try {
      ...
      if (!componentName || !componentType || !config) {
        return res.status(400).json({
          success: false,
          error:
            "Missing required fields: componentName, componentType, config",
        });
      }
      ...
      await infrastructureManager.swapComponent(
        componentName,
        config,
        componentType,
      );
  ...
  app.post("/api/ecs/create-character", async (req, res) => {
    ...
    try {
      const { playerId, characterName, position, characterClass, race } =
        req.body;
  ...
  app.get("/api/worldgen/chunk/:x/:z", async (req, res) => {
    ...
    try {
      const { ProperTerrainGenerator } = await import(
        "../game/worldgen/ProperTerrainGenerator"
      );
