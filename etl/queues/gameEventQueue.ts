...
class GameEventQueueProcessor implements IQueueProcessor {
  ...
  constructor() {
    ...
    // Initialize workers
    this.playerActionWorker = new Worker<PlayerActionJob>('playerActions', 
      this.processPlayerAction.bind(this), 
      { connection: this.connection }
    );

    this.worldUpdateWorker = new Worker<WorldUpdateJob>('worldUpdates', 
  ...
  private setupEventListeners(): void {
    this.playerActionWorker.on('completed', (job) => {
      logger.info('Player action job completed', {
    ...
    this.playerActionWorker.on('failed', (job, error) => {
      logger.error('Player action job failed', error, {
        service: 'GameEventQueue',
        jobId: job?.id,
        playerId: job?.data?.playerId,
        actionType: job?.data?.actionType
    ...
    this.worldUpdateWorker.on('completed', (job) => {
      logger.info('World update job completed', {
        service: 'GameEventQueue',
        jobId: job.id,
    ...
    this.worldUpdateWorker.on('failed', (job, error) => {
      logger.error('World update job failed', error, {
        service: 'GameEventQueue',
        jobId: job?.id,
        regionId: job?.data?.regionId,
        updateType: job?.data?.updateType
  ...
  async addPlayerAction(actionData: Omit<PlayerActionJob, 'id' | 'timestamp'>): Promise<void> {
    ...
    logger.debug('Player action added to queue', {
      service: 'GameEventQueue',
      jobId: jobData.id,
  ...
  async addWorldUpdate(updateData: Omit<WorldUpdateJob, 'id' | 'timestamp'>): Promise<void> {
    ...
    logger.debug('World update added to queue', {
      service: 'GameEventQueue',
      jobId: jobData.id,
  ...
  async processWorldUpdate(job: Job<WorldUpdateJob>): Promise<void> {
    ...
    logger.debug('Processing world update', {
      service: 'GameEventQueue',
      jobId: data.id,
      regionId: data.regionId,
  ...
  async close(): Promise<void> {
    ...
    logger.info('Game event queue system closed', { service: 'GameEventQueue' });
  }
}
