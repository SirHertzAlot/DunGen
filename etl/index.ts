...
class ETLService {
    ...
    this.app.post('/events/ingest', async (req, res) => {
      try {
        ...
        switch (eventType.split('.')[0]) {
          case 'player':
            transformedData = await playerTransformer.transform(eventType, data);
            break;
          case 'world':
        ...
        logger.error('Event ingestion failed', { error: error.message, body: req.body });
  ...
  async initialize() {
    ...
    try {
      ...
      logger.error('Failed to initialize ETL service', { error: error.message });
      throw error;
    }
  }
